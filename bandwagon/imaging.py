...
# 필요할 때 처음부터 다시 계산'한다.
#   - pristine_orig: 마지막 '새 원본' 시점 이미지(로드 시/WB 적용 시)
...
#
# wb_composite는 특수하다 — 입력을 변형하는 게 아니라 가시광+UV를 합쳐
# 완전히 새 이미지를 만든다. 재생 중 이 연산을 만나면 그 결과를 새로운
# '현 시점 원본'으로 놓고 이후 연산을 거기서부터 이어간다.
...
def apply_edit_op(img, gray_override, op_name, params):
    ...
    if op_name == "wb_composite":
        # 새 '현 시점 원본'을 만드는 특수 연산 — img/gray_override 입력은
        # 무시하고 완전히 새로 만든다(그래서 이 연산이 재생 중 나오면 그 뒤
        # 연산들은 이 결과를 기준으로 이어진다).
        vis_img, uv_img, corners, opacity = (params["visible_img"], params["uv_img"],
                                              params["corners"], params["opacity"])
        warped = warp_uv_to_visible(uv_img, np.array(corners, dtype=np.float32), vis_img.size)
        blended = blend_visible_uv(vis_img, warped, opacity)
